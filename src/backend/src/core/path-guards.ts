...
import type { DirectoryEntry } from "@backend/types/project.js";
...
export async function resolveUserFilePath(projectRoot: string, userRelativePath = ""): Promise<string> {
  ...
  if (sanitized.startsWith(".work.history") || sanitized.startsWith("runs")) {
    throw Object.assign(new Error("ZeroShot run output is not editable through the editor API"), { statusCode: 403 });
  }
...
export async function listDirectoryEntries(
  ...
  const mapped = await Promise.all(
    entries.map(async (entry) => {
      if (options?.hideWorkHistory && (entry.name === ".work.history" || entry.name === "runs")) {
        return null;
      }
