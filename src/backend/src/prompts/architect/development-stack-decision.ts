      ...
      options: [
        {
          ...
          productRequirement: "구현은 TypeScript를 기본 언어로 사용하고, 프론트엔드는 src/ui/src 구조를 따르며 백엔드는 src/server/src 구조를 따르는 풀스택 애플리케이션으로 구성해야 합니다. Bootstrap: --type fullstack --server-language typescript --ui-language typescript --profile standard."
        },
        {
          id: "python-fastapi-react",
          label: "Python FastAPI + React",
          detail: "백엔드는 Python API, 프론트엔드는 React로 구성합니다.",
          productRequirement: "구현은 Python FastAPI 백엔드를 src/server 아래에 두고 React 프론트엔드를 src/ui 아래에 두는 풀스택 애플리케이션으로 구성해야 합니다. Bootstrap: --type fullstack --server-language python --ui-language typescript --profile standard."
        },
        {
          id: "go-api-react",
          label: "Go API + React",
          detail: "성능과 단순한 배포를 우선해 Go 백엔드를 사용합니다.",
          productRequirement: "구현은 Go 백엔드를 src/server 내부의 cmd/server와 internal 구조로 구성하고 React 프론트엔드를 src/ui 아래에 둬야 합니다. Bootstrap: --type fullstack --server-language go --ui-language typescript --profile standard."
        },
        {
          id: "rust-api-react",
          label: "Rust API + React",
          detail: "비동기 안정성과 타입 안전성을 강하게 가져갑니다.",
          productRequirement: "구현은 Rust 백엔드를 src/server 아래에 두고 tokio 기반 비동기 처리를 고려하며 React 프론트엔드를 src/ui 아래에 둬야 합니다. Bootstrap: --type fullstack --server-language rust --ui-language typescript --profile standard."
        },
        {
          id: "backend-first",
          label: "백엔드 우선",
          detail: "화면보다 API, 데이터 모델, 실행 흐름을 먼저 만듭니다.",
          productRequirement: "구현은 백엔드 중심으로 진행하고 scaffold된 api, core, integrations, services, models, config, common 책임을 명확히 분리해야 합니다. Bootstrap: --type backend --language typescript --profile standard."
        }
      ]
    };
  }
