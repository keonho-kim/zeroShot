...
function modeLabel(job: JobSnapshot | null): string {
  return job?.mode === "update" ? "Update" : "Build";
}

function titleForPhase(phase: string, label: string): string {
  if (phase === "job_started") {
    return `Start ${label.toLowerCase()} session`;
  }
  if (phase === "job_finished") {
    return `Finish ${label.toLowerCase()} session`;
  }
  if (phase === "job_failed") {
...
function buildCards(job: JobSnapshot | null, logs: LogLine[]): WorkCard[] {
  ...
  return ordered.map((phase, index) => {
    ...
    return {
      id: phase,
      title: titleForPhase(phase, label),
      status,
      summary: latest,
      lines
    };
...
export function BuildFlashcards({ job, logs }: Props) {
  ...
  return (
    <div className="flashcard-stage">
      ...
      {completionModalOpen ? (
        <div className="app-modal-backdrop" role="dialog" aria-modal="true" aria-label={`${label} completed`}>
          <Card className="app-modal">
            <button type="button" className="modal-close" aria-label="Close" onClick={() => setCompletionModalOpen(false)}>
              <X className="size-4" />
            </button>
            <p className="modal-eyebrow">{label} complete</p>
            <h2>{label} finished.</h2>
