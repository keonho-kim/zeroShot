...
flowchart LR
  ...
  B --> C["ARCHITECT/PRODUCT.html blueprint"]
  C --> D["DESIGN"]
...
- **ARCHITECT** turns a product brief into structured product decisions and an `ARCHITECT/PRODUCT.html` blueprint.
...
BUILD starts a new product implementation run for the selected project. It uses the project `ARCHITECT/PRODUCT.html` blueprint and writes user-facing run reports under `runs/`.
...
Create a deterministic language-aware project starting point before running Codex implementation work:

```bash
zeroshot bootstrap --project-root /absolute/path/to/project --type fullstack --server-language python --ui-language typescript --name mini-mani-mo
```

Bootstrap also creates `AGENTS.md`, `.agents/PROJECT_CONTEXT.md`, `.agents/skills/`, `ARCHITECT/`, and `DESIGN/`. It does not create the product blueprint itself; `ARCHITECT/PRODUCT.html` is generated later by the ARCHITECT session after the product questions are complete.

Supported project types:

| Type | Purpose |
| --- | --- |
| `backend` | Backend app or service. |
| `frontend` | Frontend app. |
| `fullstack` | Backend and frontend under `src/server` and `src/ui`. |
| `library` | Installable library/package. |
| `script` | Lightweight script-style app. |

Supported languages:

| Area | Languages |
| --- | --- |
| Backend/library/script | `python`, `typescript`, `javascript`, `go`, `rust`, `java`, `ruby`, `zig` |
| Frontend | `typescript`, `javascript` |

Bootstrap toolchain priority:

| Language | Priority |
| --- | --- |
| Python | `uv`, then native `python` |
| JavaScript/TypeScript | `bun`, then `npm` |
| Go | `go` |
| Rust | `cargo` |
| Java | `gradle`, then Maven-style boilerplate |
| Ruby | RubyGems-compatible boilerplate |
| Zig | `zig` |
...
| Area | Defaults |
| --- | --- |
| Python dev | `ruff`, `ty`, `pytest`, `pytest-asyncio` |
| Python LLM profile | `fastapi`, `uvicorn[standard]`, `sse-starlette`, `langchain`, `langgraph`, `deepagents`, `langchain-mcp-adapters`, `fastmcp`, `a2a-sdk` |
| Rust backend | `tokio` with full features, `serde`, `serde_json`, `thiserror`, `tracing`, `tracing-subscriber` |
| React frontend | React, Vite, TypeScript, Floating UI, Radix Slot, TanStack Query, Lexical, Framer Motion, Sentry, Zustand, Tailwind utilities |

Common options:

| Option | Purpose |
| --- | --- |
| `--project-root <path>` | Required target project directory. |
| `--type <type>` | Project type. Defaults to `backend`. |
| `--language <language>` | Language for non-fullstack projects. |
| `--server-language <language>` | Server language for fullstack projects. |
| `--ui-language <language>` | UI language for fullstack projects. |
| `--name <name>` | Project/package name. Defaults to the directory name. |
| `--module <module>` | Module path or package namespace for Go, Java, Python, and Ruby. |
| `--python <version>` | Python minor version for uv/native Python. Defaults to `3.14`. |
| `--profile <profile>` | Dependency profile. Use `llm` for LLM/agent-oriented Python defaults. |
| `--skip-init` | Create the ZeroShot folders and source layout without package manager/toolchain init, dependency install, or package metadata files. |
| `--dry-run` | Print the planned bootstrap actions without changing files. |
...
UPDATE compares `ARCHITECT/PRODUCT.html`, `UPDATE.md`, and the previous run history, then implements the requested changes. It accepts the same shared options as `build`.
...
| --- | --- |
| `ARCHITECT/PRODUCT.html` | Product blueprint generated by ARCHITECT |
| `DESIGN/index.html` | Interactive design artifact generated by DESIGN |
| `runs/` | User-facing build/update run reports |
...
Run the server and UI separately:
