...
BUILD starts a new product implementation run for the selected project. It uses the project `PRODUCT.html` blueprint and writes user-facing run reports under `runs/`.
...
Bootstrap also creates `AGENTS.md`, `.agents/PROJECT_CONTEXT.md`, `.agents/skills/`, `PRODUCT.html`, and `DESIGN.md`. `PRODUCT.html` is the canonical product blueprint; ZeroShot no longer creates `PRODUCT.md`.
...
| `--skip-init` | Create ZeroShot boilerplate without running language init commands. |
