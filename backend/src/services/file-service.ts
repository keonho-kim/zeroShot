...
export async function writeProjectDocument(projectRoot: string, filename: "PRODUCT.md" | "UPDATE.md" | "DESIGN.md" | "DESIGN.runtime.json", content: string): Promise<void> {
...
export async function writeProductOrUpdate(projectRoot: string, filename: "PRODUCT.md" | "UPDATE.md", content: string): Promise<void> {
  await writeProjectDocument(projectRoot, filename, content);
}
